...
if(isset($_POST['save']))
{
  ...
  // validate the category name, product name and price
  if(!preg_match('/^[a-zA-Z ]+$/',$category)){
    echo '<script>alert("Your category name should only be alphabets.")</script>';
  }elseif(!preg_match('/^[a-zA-Z ]+$/',$product)){
    echo '<script>alert("Your product name should only be alphabets.")</script>';
  }elseif(!is_numeric($price) && !is_float($price)){
    echo '<script>alert("Your price should only be numeric")</script>';
  }else{
                      ...
                      { 
                        ...
                        <tr>
                          ...
                          <td class=" text-center"><a href="#"  class=" edit_data4" id="<?php echo  ($row->id); ?>" title="click to edit"><i class="mdi mdi-pencil-box-outline text-success" aria-hidden="true"></i></a>
                            ...
                            <a href="product.php?del=<?php echo ($row->id);?>" data-toggle="tooltip" data-original-title="Delete" onclick="return confirm('Do you really want to delete?');"> <i class="mdi mdi-delete text-danger"></i> </a>
                          </td>
                        </tr>
                        <?php 
                        $cnt=$cnt+1;
